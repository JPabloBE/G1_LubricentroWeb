...
El deploy se hace directamente con el CLI de Railway — **no a través de git push**:

```bash
railway up --service LubricentroWeb -d -m "descripción del cambio"
```

- `-d` corre en background (detached)
- `-m` es el mensaje del deploy
- No se usa `git push` ni CI/CD automático; el comando sube el directorio local directamente
- Siempre verificar que el servidor local funciona antes de hacer deploy
